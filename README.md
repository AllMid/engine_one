Будет такой CHANGELOG или типа того.

пока напоминалки:
1. Где undef на конце файла - это файлы, в которых может быть что-то полезное 
   на будущее, но с проектом напрямую сейчас они не связаны.
2. Если будут какие-то системные функции, связаные с ресурсами(текстуры, звук, шрифт и т.д., что нужно будет держать в памяти), то
   предлагаю использовать Asset_manager.
3. В main будет только запуск движка игры(или пародии на него)
4. Пока файлы ресурсов лежат там же, где и фалы с кодом, но позже надо будет создать для них отдельную папку.
5. На данный момент почти ничего не готово. Пока получается только запустить окошко с текстуркой(аля "Бэкграунд").
6. Если что-то изменится, то следует обновить этот список
...
Списки идей:
1. Будет паркурный 2D слэшер
2. графика Пиксельная, высокодетализированная
3. главная механика - Слэш(рывок с нанесением урона во врем действа)
4. Вариант развития - х2(или х1.5) При повторном нажатии кнопки слэша во время работы механики(ориентир-сверкание)
5. ГГ: Заяц-самурай(и этим все сказано)
6. Механика прыжка будет
7. Будет механика двойного прыжка(При контакте с поверхностью повторяется прыжок со сменой направления на Y(еще не решено) градусов)
